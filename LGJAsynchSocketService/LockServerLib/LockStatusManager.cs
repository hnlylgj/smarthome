...
namespace LGJAsynchSocketService.LockServerLib
{
     public class LockStatusManager
    {
        ...
        //string MessageMobileStr;
         ...
         Byte LockStatusID;
         public LockStatusManager(AsynchLockServerSocketService InAsynchLockServerSocketService, LoginUser MeLoginUser, int MeRecieveCount)
        {
         ...
         public LockStatusManager(AsynchLockServerSocketService InputAsynchSocketServiceBaseFrame, SocketServiceReadWriteChannel InputReadWriteObject, int InputRecieveCount)
        {
         ...
         public void CompleteCommand()
         {
             ...
             LockStatusID = MyReadBuffers[22];
             //MessageKeyIDStr = string.Format("{0:D2}", MyReadBuffers[22]);


             ReplyMessageToMobile();
             ReplyLoginMessageToLock(0);
                         
         }

         private void ReplyMessageToMobile()
         {

             string CommandMessageIDStr="lockstatus";
             string CommandMessageParaStr;
             switch (LockStatusID)
             {
                 case 0:
                     CommandMessageParaStr = "openlock";
                     break; 
                 case 1:
                     CommandMessageParaStr = "closelock";
                      break; 
                 case 2:
                      CommandMessageParaStr = "antilock";
                      break; 
                 default:
                      CommandMessageParaStr = "howlock";
                     break; 
             }


             string MyTimerStr = GetDateTimeWeekIndex();


             string CommandMessageStr = CommandMessageIDStr + "#" + this.MyLoginUser.MobileID + "-" + this.MyLoginUser.LockID + "#[" + CommandMessageParaStr + "," + MyTimerStr + "]!";
